...
bool CPCTable::SetTableData(void* pvTable, WCHAR* pwszSheetName, std::wstring* pstrDataName, BSTR bstrData)
{
	...
	if (0 == wcscmp(pwszSheetName, L"Table_Data_TW"))
	{
		...
		{
			CheckNegativeInvalid( pstrDataName->c_str(), bstrData );
			pPC->wBasic_LP = READ_DWORD( bstrData, (DWORD)pstrDataName->c_str() );
		}
		...
		else if (0 == wcscmp(pstrDataName->c_str(), L"Basic_AP"))
		{
			CheckNegativeInvalid(pstrDataName->c_str(), bstrData);
			pPC->unknown2 = READ_DWORD(bstrData, (DWORD)pstrDataName->c_str());
		}
		...
		{
			CheckNegativeInvalid( pstrDataName->c_str(), bstrData );
			pPC->byStr = READ_WORD( bstrData, pstrDataName->c_str() );
		}
		else if (0 == wcscmp(pstrDataName->c_str(), L"Basic_Con"))
		{
			CheckNegativeInvalid( pstrDataName->c_str(), bstrData );
			pPC->byCon = READ_WORD(bstrData, pstrDataName->c_str());
		}
		else if (0 == wcscmp(pstrDataName->c_str(), L"Basic_Foc"))
		{
			CheckNegativeInvalid( pstrDataName->c_str(), bstrData );
			pPC->byFoc = READ_WORD(bstrData, pstrDataName->c_str());
		}
		else if (0 == wcscmp(pstrDataName->c_str(), L"Basic_Dex"))
		{
			CheckNegativeInvalid( pstrDataName->c_str(), bstrData );
			pPC->byDex = READ_WORD(bstrData, pstrDataName->c_str());
		}
		else if (0 == wcscmp(pstrDataName->c_str(), L"Basic_Sol"))
		{
			CheckNegativeInvalid( pstrDataName->c_str(), bstrData );
			pPC->bySol = READ_WORD(bstrData, pstrDataName->c_str());
		}
		else if (0 == wcscmp(pstrDataName->c_str(), L"Basic_Eng"))
		{
			CheckNegativeInvalid( pstrDataName->c_str(), bstrData );
			pPC->byEng = READ_WORD(bstrData, pstrDataName->c_str());
		}
		...
		else if (0 == wcsncmp(pstrDataName->c_str(), L"Radius_", wcslen(L"Radius_")))
		{
			...
			for (int i = 0; i < 13; i++)
			{
				...
				{
					pPC->unknown1[i] = READ_FLOAT(bstrData, pstrDataName->c_str());
...
{
	if (byRace > RACE_COUNT)
	{
		return NULL;
	}

	if (byClass > PC_CLASS_COUNT)
	{
		return NULL;
	}

	if (byGender > GENDER_COUNT)
	{
		return NULL;
	}
