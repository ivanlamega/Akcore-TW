...
int CNtlPacketEncoder_RandKey::RxDecrypt(void * pvPacketHeader)
{
	...
	{
		Decrypt((BYTE*)pPacketHeader, PACKET_HEADSIZE, m_rxHeaderKeyGenerator.Generate(), (BYTE*)CHECKSUM_INITIAL_VALUE);
...
int CNtlPacketEncoder_RandKey::RxDecrypt(CNtlPacket& rPacket)
{
	...
	// Checksum
	NTL_PRINT(PRINT_SYSTEM, "Rx PacketLen[%u] Sequence[%u] Checksum[%u]", rPacket.GetPacketHeader()->wPacketLen, 0, CHECKSUM_INITIAL_VALUE);
	//if (byCheckSum != CHECKSUM_INITIAL_VALUE)
	//{
